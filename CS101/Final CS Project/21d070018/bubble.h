...
public:
    ...
    void nextStep(double t)
    {
        ...
        if ((vy < 0 && new_y < (20 + circle.getRadius())) // bounce along X direction at left border
            ||
            (vy > 0 && new_y > (450 - circle.getRadius()))) // bounce along X direction at right border
        {
    ...
    void change_velocity(double vx_, double vy_){ //An extra function we added to change the velocity on hitting obstacles
        vx=vx_;
        vy=vy_;
        return;
    }
