...
vector<Bubble> create_bubbles(int level)
{
    ...
    for(int i=1; i<=level; i++){
        bubbles.push_back(Bubble(WINDOW_X*(double)i/(level+1), BUBBLE_START_Y, pow(2,level)*BUBBLE_DEFAULT_RADIUS, pow(-1,i)*((double)level/i)*BUBBLE_DEFAULT_VX, 0, COLOR((level*138)%255,100,18*(level+1)%255)));
    }
    return bubbles;
}
...
int main()
{
    ...
    for(int level_no=1; ; level_no++){ //The for loop made for levels
        ...
        while (true)
        {
            ...
            int actual_time_left=time_limit- t/30;
