...
int main(){
    ...
    for(int i=0; i<n; i++){
        int init_bal;
        std::cin >> init_bal;
        if((bank.add_user(init_bal)) == -1){ //sneaky way of addiing initial amount
            std::cout << "Error: Max users reached" << std::endl;
        }
    } //No problemo until here
    ...
    } //Everything going changa
    // bank.print_requests();

    // process requests
    for(int i=0; i<bank.n_reqs; i++){
        Request req = bank.reqs[i]; //We take one req from queue
        if(req.is_deposit){ //If that is a deposit
